...
class LabelTFPanel extends Panel{
	...
	LabelTFPanel(String lbl,String TF,FocusListener e){
		...
		tf1 = new TextField(TF);l1.setText(lbl);
		tf1.addFocusListener(e);add(l1);add(tf1);
	}
}
...
class LabelChoicePanel extends Panel{
	...
	LabelChoicePanel(String lbl, String[] Choice){
		...
		setLayout(new FlowLayout(FlowLayout.CENTER));
		l1.setText(lbl);
		for(String a: Choice)	c1.add(a);add(l1);add(c1);
	}
}
...
class AWT extends Frame implements FocusListener,ItemListener{
	...
	AWT(){
		...
		left.add(regno);left.add(name);
		genderFrame.add(male);genderFrame.add(female);
		left.add(genderFrame);left.add(degree);left.add(year);left.add(br);
		dob = createDOB();left.add(dob);
		address.add(new Label("Address"));
		address.add(addr);left.add(address);
		email.add(new Label("Email     "));
		email.add(emailTF);left.add(email);
		hobby.add(stamp);hobby.add(novel);
		hobby.add(tennis);right.add(hobby);
		exlist.add("Tennis");exlist.add("Cricket");
		exlist.addItemListener(this);
		excurr.add(exlist);right.add(excurr);op.add(output);
		right.add(op);
	...
	Panel createDOB(){
		Panel a = new Panel();
		a.add(new Label("DOB"));
		for (int i =1; i <=31; i++){d.add(i+"");}
		for (int i =1; i <=12; i++){m.add(i+"");}
		for (int i =1990; i <=2018; i++){y.add(i+"");}
		a.add(d);a.add(m);a.add(y);
		return a;
	}
	public void itemStateChanged(ItemEvent e){
		...
		if(stamp.getState()){hob += "Stamp Collection";}
		if(novel.getState()){hob+=",Reading Novel";}
		if(tennis.getState()){hob+=",Playing Tennis";}
		String noun,salu;
		if(male.getState()){noun = "He";salu = "Shri";}
		else{noun = "She";salu = "Sow";}
		...
		output.setText(op);output.setEditable(false);
	}
}

public class StudentResponse{
	public static void main(String[] args) {AWT a;a = new AWT();}
}
